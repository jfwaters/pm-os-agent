...
| **Token / cost budget** | $0.50 hard cap per run (enforced in-loop); $20/day account hard cap (enforced via a dated cross-run spend ledger, checked at run start and mid-run); plus a provider-dashboard spend limit as the outer backstop | An overnight runaway bill from a loop or a hook-fire storm | ✅ yes |
...
> **Mind the gap:** every above-the-line item in the M1 map has a checkpoint, so there is no *decision* gap. But the agent line is only real once enforced: max-iterations, revisions, data-pull attempts, per-run cost cap, the queue cap, the **timeout**, the **kill switch**, and the **$20/day cap** are enforced in code today — 8 of 9. Only **JIT single-use tokens** remain specified-but-not-implemented, because they need a real write backend to authorize against; in the current mock build there is nothing to mint a scoped credential for.
...
| **Memory drift / poisoning** | A stored fact diverges from a fresh pull; a figure not traceable to current data; provenance mismatch | Re-fetch volatile sources every run (no caching) and the critic's `INVENTED_DATA` check are **live**; validate-on-read against a live `get_project` and provenance tags are **planned** (Cortex persists no semantic facts yet) |
...
> **Status:** the six EV cases are specified and runnable by hand against the fixtures today (the mock tools and `CORTEX_WITHHOLD` make the tool layer deterministic). The **CI gate and an automated replay harness are the plan, not yet wired** — this section is the lifecycle we're building toward, not a pipeline that exists.
...
Two things this exercise made concrete. First, **most of Cortex's "pass" conditions are stops, not answers** — refuse, escalate, halt. The bounds aren't a safety afterthought bolted onto the agent; they *are* the behavior, because the agent line is only real once a number enforces it. Second, writing the bounds table exposed the **paper-vs-enforced gap** and then helped close most of it: **eight of the nine bounds are now live in code** (max-iterations, revisions, data-pull attempts, per-run and daily cost caps, queue cap, timeout, kill switch), and only **JIT single-use tokens** remain policy — they need a real write backend to authorize against, which the mock build doesn't have. Naming the gap honestly, and then shrinking it, beats pretending the table is the build.
